...
#
#     assert board_list[1]["name"] == "Board_4"


def test_create_board(base_url: str, org_id: str, api_key: str, token: str, test_data):
    name_board = test_data.get("board_name")
    api = BoardApi(base_url, api_key, token, org_id)
...
def test_delete_board(base_url: str, org_id: str, api_key: str, token: str, test_data):
    name_board = test_data.get("board_name")
    api = BoardApi(base_url, api_key, token, org_id)
    api.create_board(name_board)
    board_list_before = api.get_all_boards_by_org_id()
...
def test_delete_board_by_id(base_url: str, org_id: str, api_key: str, token: str):
    api = BoardApi(base_url, api_key, token, org_id)
    board_list = api.get_all_boards_by_org_id()
    new_id = board_list[-1]["id"]
    api.delete_board(new_id)
